...
score_cpu = -gmm_cpu.score(rgb).reshape(608, 800)
print score_cpu.min(), score_cpu.max()

plt.subplot(1, 2, 1)
plt.imshow(score_cpu, interpolation='nearest', vmin=0, vmax=96)

score_cpu[:] = 0
dScore = cl.Buffer(context, cm.READ_ONLY | cm.COPY_HOST_PTR, hostbuf=score_cpu)

gmm.score(dSrc, dScore)
cl.enqueue_copy(queue, score_cpu, dScore).wait()
print score_cpu.min(), score_cpu.max()
